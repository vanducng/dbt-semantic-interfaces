...
class DunderedNameFormatter:
    ...
    @staticmethod
    def parse_name(name: str) -> DunderedNameFormatter:
        """Construct from a string like 'listing__ds__month'."""
        name_parts = name.split(DUNDER)

        # No dunder, e.g. "ds"
        if len(name_parts) == 1:
            return DunderedNameFormatter((), name_parts[0])
        ...
        if associated_granularity:
            #  e.g. "ds__month"
            if len(name_parts) == 2:
                return DunderedNameFormatter((), name_parts[0], associated_granularity)
            # e.g. "messages__ds__month"
            return DunderedNameFormatter(
